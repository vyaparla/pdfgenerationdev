module Report
  ...
  private
    ...
    def summary_table_data
     ...
     building_floors = Lsspdfasset.select(:u_building, :u_floor, :u_type, :u_other_floor).where(:u_facility_id => @owner.u_facility_id, :u_report_type => ["DAMPERREPAIR" ,"DAMPERINSPECTION"],  :u_building => @building, :u_delete => false).where.not(u_type: "").pluck(:u_floor)
     building_other_floors = Lsspdfasset.select(:u_building, :u_floor, :u_type, :u_other_floor).where(:u_facility_id => @owner.u_facility_id, :u_report_type => ["DAMPERREPAIR" ,"DAMPERINSPECTION"], :u_building => @building, :u_delete => false).where.not(u_type: "").pluck(:u_other_floor)
    ...
    def calculate_status_when_no_common_floor_data(key)
          if  key[1] == "other"
     ...
     def calculate_type_when_no_common_floor_data(key)
         if  key[1] == "other"
    ...
    def for_type_calculate_non_integer_floor_values(building_floors, building_other_floors, floor_data, floor_json, key, value, buildingInfo)
        if building_floors & building_other_floors == []
           building_result = calculate_type_when_no_common_floor_data(key)
         else
