module Report
  ...
  private
    ...
    def summary_table_data
      ...
      @buildingInfo = Lsspdfasset.select(:u_building, :u_floor, :u_type).where(:u_service_id => @owner.u_service_id, :u_building => @building, :u_delete => false ).group(["u_building", "u_floor", "u_type"]).order(:u_floor).count(:u_type)
      @floorInfo = []
