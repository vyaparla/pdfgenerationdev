module DamperRepairReport
  ...
  private
    ...
    def dr_project_summary_table_data
      @dr_project_summaryInfo = Lsspdfasset.select(:u_building, :u_type, :u_dr_passed_post_repair).where(:u_service_id => @job.u_service_id, :u_delete => false).where.not(u_type: "").group(["u_building", "u_type", "u_dr_passed_post_repair"]).order("CASE WHEN u_type = 'FD' THEN '1' WHEN u_type = 'SD' THEN '2' ELSE '3' END").count(:u_dr_passed_post_repair)
      @dr_buildingInfo = []
